//Version_JS;Version_WBotCeMin;Version_CEF4Min;
//3.1.4.0;0.1.1.2;120.1.10
...
(function() {
    ...
    // Função que varre os módulos internos do WhatsApp Web (Webpack)
    ...
    // 1. Procurando o módulo 'User' (que contém getMe)
    findModule((m) => m.getMe && m.getUserWid).then(UserModule => {
        if (UserModule) {
            window.Store.User = UserModule;
            console.log('Módulo Store.User (getMe) encontrado.');
        }
    });

    // 2. Procurando o módulo 'WidFactory'
    findModule((m) => m.createWid && m.isWid).then(WidFactoryModule => {
        if (WidFactoryModule) {
            window.Store.WidFactory = WidFactoryModule;
        }
    });

    // 3. Procurando o módulo 'Msg' (para criar a mensagem)
    findModule((m) => m.create && m.type).then(MsgModule => {
        if (MsgModule) {
            window.Store.Msg = MsgModule;
        }
    });

    // 4. Procurando o módulo 'MsgKey' (para criar o ID da mensagem)
    findModule((m) => m.newId && m.fromString).then(MsgKeyModule => {
        if (MsgKeyModule) {
            window.Store.MsgKey = MsgKeyModule;
        }
    });

    // 5. Procurando o módulo 'Chat' (para encontrar a conversa)
    findModule((m) => m.find && m.get).then(ChatModule => {
        if (ChatModule) {
            window.Store.Chat = ChatModule;
        }
    });
    
    // 6. Procurando o módulo de ENVIAR MENSAGEM (addAndSendMsgToChat - O mais crítico)
...
function getMeWid() {
    if (Store.User && typeof Store.User.getMaybeMeUser === 'function') {
        return Store.User.getMaybeMeUser();
    }

    if (Store.Conn && Store.Conn.me) {
        return Store.Conn.me;
    }
...
function createWidSafe(id) {
    try {
        if (window.WidFactory?.createWid) {
...
function getMeWidSilent() {
    try {
        return Store.User.getMeUser()?.wid || null;
    } catch (e) {
        return null;
    }
}
...
window.WAPI.createGroup = async function (groupName, participantsIds) {
    ...
    try {
        ...
        const participants = participantsIds
            .map(createWidSafe)
            .filter(wid => wid && !wid.equals(meWid));

        if (!participants.length) return null;


        const result = await Store.GroupMetadata.create(
            groupName,
            participants
        );
        ...
        (result.participants || []).forEach(p => {
            ...
            users[wid] = {
                wid,
                code: p.error ? Number(p.error) : 200,
                invite_code: p.invite_code || null,
                invite_code_exp: Number(p.invite_code_exp) || null
            };
        });
...
//gutembergscosta - 25/11/2025
window.WAPI.extractRealNumberFromContact = function (info) {
    let contact = Store.Contact.get(info.id);
	
	if (!contact) return null;

    // Multi-device → número real armazenado neste campo interno
    if (contact.__x_phoneNumber && contact.__x_phoneNumber._serialized) {
		return contact.__x_phoneNumber;
    }

    // Legado → phoneNumber tradicional
    if (contact.phoneNumber && contact.phoneNumber._serialized) {
	return contact.phoneNumber;
    }

    // Como último recurso: se já for c.us, é o número real
    ...
    try {
        ...
        if (chatId.includes('@lid')) {
            ...
            if (lidMatch && lidMatch[1]) {
                normalizedChatId = lidMatch[1] + '@c.us';
            }
        }
        ...
        let searchPaths = [
            ...
            () => {
                try {
                    ...
                    return wid ? window.Store.Chat.get(wid) : null;
                } catch { return null; }
            },
            () => {
                try {
            ...
            () => {
                ...
                return allChats.find(c => {
                    const chatIdStr = c.id?._serialized || c.id || '';
                    return chatIdStr === normalizedChatId || 
                           chatIdStr === chatId ||
                           chatIdStr.includes(chatId.replace('@lid', ''));
                });
            }
        ];
        ...
        for (const tryFind of searchPaths) {
            try {
                const result = typeof tryFind() === 'object' && 'then' in tryFind() 
                    ? await tryFind() 
                    : tryFind();
        ...
        if (window.Store.ReadSeen && window.Store.ReadSeen.sendSeen) {
            try {
                await window.Store.ReadSeen.sendSeen(chat, true); // true = enviar para servidor
                serverMarked = true;
            } catch (e) {
        ...
        if (!serverMarked && window.Store.SendSeen && window.Store.SendSeen.sendSeen) {
            try {
        ...
        if (!serverMarked && chat.sendSeen && typeof chat.sendSeen === 'function') {
            try {
                await chat.sendSeen();
                serverMarked = true;
            } catch (e) {
        ...
        if (!serverMarked) {
            try {
                ...
                const lastMsg = chat.msgs && chat.msgs._models 
                    ? chat.msgs._models[chat.msgs._models.length - 1]
                    : null;
                ...
                if (lastMsg && lastMsg.id) {
                    ...
                    if (window.Store.Msg.sendRead) {
                        await window.Store.Msg.sendRead(ackData);
                        serverMarked = true;
                    }
                    else if (window.Store.Conn && window.Store.Conn.sendMessageAck) {
                        await window.Store.Conn.sendMessageAck(ackData);
                        serverMarked = true;
                    }
                }
        ...
        if (!serverMarked) {
            ...
            try {
                if (window.Store.Sync && window.Store.Sync.sendChatState) {
                    await window.Store.Sync.sendChatState(chat.id, 'read');
                }
            } catch (e) {
            }
        }
        ...
        setTimeout(() => {
            try {
                if (chat.trigger) {
                    chat.trigger('change:unreadCount', 0);
                    chat.trigger('change');
                }
                
                if (window.Store.AppState) {
                    window.Store.AppState.emit('chat.update', {
                        id: chat.id,
                        unreadCount: 0
                    });
                }
...
window.WAPI.verifyAndMarkAsRead = async function(chatId) {    
    const normalizedId = window.WAPI.normalizeChatId(chatId);
    
    const chat = window.Store.Chat.get(normalizedId);
    if (!chat) {
        
        const contact = window.Store.Contact.get(normalizedId);
        if (contact) {
 
        }
    }
    
    return await window.WAPI.sendSeen(chatId);
};
...
window.WAPI.getUnreadMessages = function(includeMe, includeNotifications, use_unread_count, done) {
    ...
    for (let chat in chats) {
		...
		//( Gabriel Rocha - 05/12/2025 Mudar o id do chat de lid para telefone
...
window.WAPI.sendImage = async function (imgBase64, chatid, filename, caption) {
    try {
        ...
        // Se for contato @c.us, precisamos buscar o JID primeiro
        if (chatid.includes('@c.us')) {
            const contact = await window.WAPI.findJidFromNumber(chatid);
            if (!contact || contact.status === 404) {
                console.log('Contato não encontrado em findJidFromNumber:', chatid);
                return false;
            }
            chat = await window.WAPI.findChatFromId(contact.jid);
        } else {
            // Para grupos (@g.us) ou JIDs já válidos
            chat = await window.WAPI.findChatFromId(chatid);
        }
        ...
        // Usa MediaCollection para preparar o envio
        const mc = new Store.MediaCollection(chat);
        await mc.processAttachmentsForChat([{ file: mediaBlob }, 1], 1, chat);
        ...
        console.error('Erro em sendImage:', err);
        return false;
    }
};*/
...
//Mike 23/11/2025
window.WAPI.sendMessageToID = function (chatid, msgText, done) {
	try {
	  ...
	  if (!wid) {
		console.error('[WAPI] Erro ao criar Wid');
		if (done) done(false);
		return;
	  }

	  // Força a criação do chat com LID
	  window.Store.FindChat.findOrCreateLatestChat(wid)
		.then(function(result) {
		  
		  var chat = result.chat || result;

		  if (typeof chat.sendMessage !== 'function') {
			
			setTimeout(function() {
			  var chatAlt = window.Store.Chat.get(wid);
			  
			  if (!chatAlt || typeof chatAlt.sendMessage !== 'function') {
				chatAlt = window.Store.Chat.get(chatid);
			  }
			  
			  if (!chatAlt || typeof chatAlt.sendMessage !== 'function') {
				var allChats = window.Store.Chat.models;
				for (var i = 0; i < allChats.length; i++) {
				  var c = allChats[i];
				  if (c.contact && c.contact.id && c.contact.id._serialized === chatid) {
					chatAlt = c;
					break;
				  }
				}
			  ...
			  try {
				var sendResult = chatAlt.sendMessage(msgText);
			...
			// Chat do result tem sendMessage
			setTimeout(function() {
			  try {
				var sendResult = chat.sendMessage(msgText);
...
window.WAPI.sendImage = async function(imgBase64, chatid, filename, caption) {
    try {
        if ((chatid.includes('@c.us')) || (chatid.includes('@lid'))) {
            const contact = await window.WAPI.findJidFromNumber(chatid);
            if (!contact || contact.status === 404) {
                console.log('1. Contact not found in findJidFromNumber');
                return false;
            }

            const chat = await window.WAPI.findChatFromId(contact.jid);
            if (!chat) {
                console.log('2. Chat not found for contact');
                return false;
            }
			...
			// pega o chat diretamente do Store
			const chat = await window.WAPI.findChatFromId(chatid);
			if (!chat) {
				console.log('Grupo não encontrado:', chatid);
				return false;
			}
			...
			// usa MediaCollection como no @c.us
			const mc = new Store.MediaCollection(chat);
            ...
            console.log('4. chatid format not recognized');
            return false;
        }
    } catch (err) {
        console.error('5. Error in sendImage:', err);
        return false;
    }
};*/
...
// Mike 24/11/2025
window.WAPI.sendImage = async function (imgBase64, chatid, filename, caption, done) {
    try {
        chatid = chatid.replace(/\s/g, '').trim();
        ...
        if (!wid) {
            console.error('[WAPI] Erro ao criar Wid para imagem:', chatid);
            if (done) done(false);
            return false;
        }

        // Garante criação ou recuperação do chat
        const result = await window.Store.FindChat.findOrCreateLatestChat(wid);
        const chat = result.chat || result;

        if (!chat) {
            console.error('[WAPI] Chat não encontrado após findOrCreate:', chatid);
            if (done) done(false);
            return false;
        }
        ...
        if (!media) {
            console.error('[WAPI] Falha ao processar imagem');
            if (done) done(false);
            return false;
        }

        // delay para chats novos
        setTimeout(() => {
            try {
				media.sendToChat({
					chat: chat,
                ...
                console.error('[WAPI] Erro ao enviar imagem:', err);
                if (done) done(false);
            }
        }, 1500);
        ...
        console.error('[WAPI] Erro geral no sendImage:', err);
        if (done) done(false);
        return false;
    }
};
...
window.WAPI.onIncomingCallCallback = async function() {
	const call = window.Store.Call && window.Store.Call._models && window.Store.Call._models[0];
    if (!call) return;

    // tenta pegar o contato direto da call
    let contact = call.contact || call.__x_contact || null;

    // se não vier, tenta resolver via peerWid/peerJid
    if (!contact) {
	...
	// Usando a função de gutembergscosta 
    let real = window.WAPI.extractRealNumberFromContact(contact)._serialized;

    // se vier algo tipo 55119xxxxxxx@c.us, tira o sufixo
    if (real && real.endsWith('@c.us')) {
        real = real.split('@')[0];
    }

    // aqui você manda pro Delphi só o número real, não o LID
